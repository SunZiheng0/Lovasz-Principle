...
def argument():
    ...
    # GINDataset
    parser.add_argument(
        "--dataset", type=str, default="GINDataset", help="Name of dataset."
    )
    ...
    # loss parameter
    parser.add_argument(
        "--reg_adj", type=float, default=0.5, help="regularization for A"
    )

    parser.add_argument(
        "--reg_z", type=float, default=0.5, help="regularization for kernels"
    )
...
if __name__ == "__main__":
    ...
    emb, _ = model.get_embedding(wholegraph, wholefeat)
    emb = emb.cpu()
    ...
    for epoch in range(args.epochs):
        ...
        for graph, label in dataloader:
            ...
            optimizer.zero_grad()
            loss = model(args, graph, feat, graph_id)
            loss.backward()
            optimizer.step()
            loss_all += loss.item()
